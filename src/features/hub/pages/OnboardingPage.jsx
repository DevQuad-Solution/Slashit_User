...
export function Onboarding() {
  ...
  const finish = async (hubArg) => {
    ...
    const email = session?.email || Cookies.get("slashit_signup_email") || "";
    const password = Cookies.get("slashit_signup_pw") || "";
    const verifyToken = Cookies.get("slashit_verify_token") || "";
    if (!hub) return;
    ...
    if (email && verifyToken && verifyToken.length > 20) {
      try {
        const onboardRes = await api.auth.onboarding(
          verifyToken,
          email,
          hub.id,
        );
    ...
    if (onboardingToken) {
      ...
      const _mapped = mapUser(
        { email, name: prev.name || "" },
        {
          ...prev,
          hubId: hub.id,
          hubName: hub.name,
      ...
      try {
        ...
        try {
          ...
          toast.error("Hub saved. Please sign in to continue.");
        }
      }
    }

    // Always clean up credentials after attempt
    Cookies.remove("slashit_signup_email");
    Cookies.remove("slashit_signup_pw");
    ...
    if (!signedIn) {
      ...
      Cookies.set("slashit_prefill_email", email || "");
    ...
    try {
      const pending = JSON.parse(Cookies.get("slashit_pending_join") || "null");
      if (pending?.slashId) {
        Cookies.remove("slashit_pending_join");
        nav("/slash/" + pending.slashId);
        return;
      }
    } catch (e) {}
    Cookies.set("slashit_just_onboarded", "1");
    setSaving(false);
    nav("/home");
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Pool money with other students to buy food at wholesale prices —
          saving 20–40% on every purchase.
        </div>
      </div>
      <div
        style={{
      ...
      >
        {[
          {
            icon: "🏪",
            title: "Pick a Pickup Hub",
            sub: "Get orders delivered to a verified pickup station near your campus.",
          },
          {
            icon: "🔒",
            title: "Pay Safely via Escrow",
            sub: "Your money is only released after delivery is confirmed by the hub attendant.",
          },
          {
            icon: "💰",
            title: "Save up to 40%",
            sub: "Wholesale prices, shared costs — maximum savings on food and essentials.",
          },
