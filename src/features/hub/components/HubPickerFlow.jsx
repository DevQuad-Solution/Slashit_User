...
export function HubPickerFlow({
  onComplete,
  onBack,
  title = "Set Your Pickup Location",
  sub = "Select your primary pickup station to see deals near you",
}) {
    ...
    const fetchHubs = async () => {
      ...
      try {
        ...
        for (const stateObj of statesList) {
          ...
          try {
            await sleep(1500);
            const citiesRes = await safeFetch(() =>
              api.hubs.getCities(stateName),
            );
          ...
          for (const cityObj of citiesList) {
            ...
            try {
              await sleep(2000);
              const hubsRes = await safeFetch(() =>
                api.hubs.getHubs(stateName, cityName),
              );
              ...
              /* skip this hub — missing state/city data */
            }
          }
        }
        ...
        if (hubList.length > 0) {
          ...
          try {
            const backup = JSON.parse(
              Cookies.get("slashit_hubs_backup") || "[]",
            );
  ...
  const cities =
    selState && tree[selState] ? Object.keys(tree[selState]).sort() : [];
  const hubs =
    selState && selCity && tree[selState]?.[selCity]
      ? tree[selState][selCity]
      : [];
          ...
          <>
            ...
            {!loadingHubs && states.length === 0 && (
              ...
              >
                ...
                <div
                  style={{ fontWeight: 700, color: "#1e293b", marginBottom: 6 }}
                >
                  No hubs in your area yet
                </div>
                <div style={{ fontSize: 13, lineHeight: 1.5 }}>
                  Your campus has not launched yet. Check back soon or contact
                  support.
                </div>
              </div>
            )}
            {!loadingHubs &&
              states.map((s) => (
                <LocationRow
                  ...
                  icon="📍"
                  title={s}
                  sub={`${Object.keys(tree[s]).length} ${Object.keys(tree[s]).length === 1 ? "city" : "cities"} available`}
                  onClick={() => pickState(s)}
                />
              ))}
          </>
          ...
          <>
            {cities.map((c) => (
              <LocationRow
                ...
                icon="🏙️"
                title={c}
                sub={`${tree[selState][c].length} hub${tree[selState][c].length !== 1 ? "s" : ""} available`}
                onClick={() => pickCity(c)}
              />
            ))}
          </>
          ...
          <>
            {hubs.length === 0 && (
              <div style={{ textAlign: "center", padding: 40 }}>
                ...
                >
                  There are no verified pickup stations in {selCity} yet. Try
                      ...
                      {h.transportFee !== undefined && (
                        <span
                          style={{
                            ...
                            fontWeight: 700,
                            background:
                              h.transportFee > 0 ? "#fef3c7" : "#dcfce7",
                            color: h.transportFee > 0 ? "#92400e" : "#15803d",
                            borderRadius: 6,
                            padding: "2px 7px",
                          }}
                        >
                          🚚{" "}
                          {h.transportFee > 0
                            ? `+${fmt(h.transportFee)}/slot transport`
                            : "Free transport"}
                        </span>
                      )}
                    </div>
