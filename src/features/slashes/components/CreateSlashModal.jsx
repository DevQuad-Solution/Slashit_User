...
export function CreateSlashModal({ product, user, onClose, onCreated }) {
  ...
  const TIME_OPTS = ["12h", "24h", "48h", "72h"].filter(
    (t) => hoursMap[t] <= (cfg.maxSlashDurationHours || 72),
  );
  ...
  const resolvedPrice =
    product.prices && userState && product.prices[userState] !== undefined
      ? product.prices[userState]
      : product.defaultPrice || product.pricePerSlot;
  const statePriceNote =
    product.prices && userState && product.prices[userState] !== undefined
      ? `${userState} price`
      : "base price";
  ...
  const hubTransportFee = (() => {
    ...
    try {
      const hubs = JSON.parse(Cookies.get("slashit_admin_hubs") || "[]");
      const hub = hubs.find((h) => h.id === user?.hubId);
      const prods = JSON.parse(Cookies.get("slashit_admin_products") || "[]");
      ...
      if (prod?.transport && prod.transport[user?.hubId] !== undefined) {
        return Math.round(
          Number(prod.transport[user?.hubId]) / (product.defaultSlots || 1),
        );
      }
      ...
      // 3. Platform default
      return Math.round(
        (Number(cfg.defaultTransportFee) || 0) / (product.defaultSlots || 1),
      );
    } catch (e) {
      return 0;
    }
      ...
      >
        ...
        <div style={{ padding: "0 20px 20px" }}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            >
              {product.description ||
                "Premium wholesale product split equally between all members."}
            </div>
          </div>
          ...
          >
            {[
              ["Price per slot", fmt(resolvedPrice), "#2563eb"],
              ["Total slots", product.defaultSlots, "#1e293b"],
            ].map(([l, v, c]) => (
              <div key={l}>
                <div
                  style={{
          ...
          <div style={{ marginBottom: 12 }}>
              ...
              {TIME_OPTS.map((t) => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 13,
                    fontWeight: 700,
                    border: `2px solid ${timeLimit === t ? "#2563eb" : "#e2e8f0"}`,
                    background: timeLimit === t ? "#eff6ff" : "#fff",
                    color: timeLimit === t ? "#2563eb" : "#64748b",
                  }}
                >
                  {t}
                </button>
            ...
            <div style={{ fontSize: 11, color: "#64748b", marginTop: 6 }}>
              ...
              ,{" "}
              {expiresAt.toLocaleTimeString("en-NG", {
                hour: "2-digit",
                minute: "2-digit",
              })}
            </div>
          ...
          >
            ...
            <div>
              <div style={{ fontSize: 10, color: "#94a3b8", marginBottom: 1 }}>
                This slash will be tied to your primary hub
              </div>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#1e293b" }}>
                {user?.hubName || "Bodija Hub"} — {user?.city || "Ibadan"}
              </div>
            </div>
          </div>
          ...
          >
            ...
            {[
              ...
              ...(transportFee > 0
                ? [
                    [
                      "🚚 Transport fee — " + (user?.hubName || "hub"),
                      fmt(transportFee),
                    ],
                  ]
                : []),
            ...
            <div
              style={{
                fontSize: 11,
                color: canAfford ? "#1d4ed8" : "#dc2626",
                marginTop: 6,
                fontWeight: 600,
              }}
            >
              {canAfford
                ? `💰 Wallet after: ${fmt((user?.walletBalance || 0) - totalCharge)}`
                : `⚠️ Need ${fmt(totalCharge - (user?.walletBalance || 0))} more`}
            </div>
          </div>
          ...
          >
            🔒 Payment held in escrow. Released to supplier only after hub
            attendant verifies delivery.
          </div>
