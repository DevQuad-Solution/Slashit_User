...
export function Profile() {
  ...
  useEffect(() => {
    ...
    api.admin
      .getHostels?.()
      .then((d) => {
        const list = d?.data?.hostels || d?.data || [];
        if (Array.isArray(list) && list.length > 0) setAdminHostels(list);
      })
      .catch(() => {});
  }, []);
  const liveHostels =
    adminHostels.length > 0 ? adminHostels : getAdminHostels([]);
  ...
  const openHubPicker = async () => {
    ...
    try {
      const backup = JSON.parse(Cookies.get("slashit_hubs_backup") || "[]");
      if (backup.length > 0) {
        setAllHubs(backup);
        return;
      } // already have data
    ...
    try {
      ...
      for (const s of statesList) {
        ...
        let cities = [];
        try {
          cities = (await api.hubs.getCities(stateName)).data || [];
        } catch (e) {
          continue;
        }
        for (const c of cities) {
          ...
          try {
            ...
            const hubs =
              (await api.hubs.getHubs(stateName, cityName)).data || [];
            hubs.forEach((h) =>
              hubList.push({
                ...h,
                id: h._id || h.id,
      ...
      if (hubList.length > 0) {
        setAllHubs(hubList);
        try {
          Cookies.set("slashit_hubs_backup", JSON.stringify(hubList));
        } catch (e) {}
      }
    } catch (e) {}
  ...
  const planBadge = {
    free: { label: "Basic", bg: "#f1f5f9", c: "#64748b" },
    student: { label: "Student", bg: "#dbeafe", c: "#1d4ed8" },
    premium: { label: "Premium", bg: "#fef3c7", c: "#d97706" },
  };
  const pb = planBadge[user.plan] || planBadge.free;
  const kycColors = {
    unverified: { label: "Unverified", bg: "#fee2e2", c: "#dc2626" },
    pending: { label: "Pending Review", bg: "#fef3c7", c: "#d97706" },
    verified: { label: "Verified ✓", bg: "#dcfce7", c: "#16a34a" },
    rejected: { label: "Rejected", bg: "#fee2e2", c: "#dc2626" },
  };
  ...
  const selectHostel = (h) => {
    saveUser({ ...user, hostel: h });
    setShowHostelPicker(false);
    toast.success("Hostel updated!");
  };
  // Filter hostels by user's city so only relevant hostels are shown
  const cityHostelsProfile = liveHostels.filter(
    (h) => !h.city || h.city.toLowerCase() === (user.city || "").toLowerCase(),
  );
  const filteredLiveHostels =
    cityHostelsProfile.length > 0 ? cityHostelsProfile : liveHostels;
  const hostelList = filteredLiveHostels.map((h) => ({
    name: h.name || h,
    campus: h.campus || "",
  }));
  ...
  const menuItems = [
    ...
    },
    {
      ...
      bg: "#eff6ff",
      label: "Wallet",
      sub: fmt(user.walletBalance) + " available",
      onClick: () => nav("/wallet"),
    },
    {
      icon: "🔔",
      ...
      ? [
          {
            ...
            bg: "#fef3c7",
            label: "Leaderboard",
            sub: user.hostel ? `Rank for ${user.hostel}` : "Set hostel first",
      ...
      ? [
          {
            ...
            bg: "#f0fdf4",
            label: "Referrals",
            sub: `${user.referralCount || 0} friends · ${fmt(user.referralEarnings || 0)} earned`,
    ...
    {
      icon: "🔐",
      bg: user.kycStatus === "verified" ? "#dcfce7" : "#fee2e2",
      ...
      ? [
          {
            ...
            sub: pb.label,
            badge: {
              label: user.plan === "free" ? "Upgrade" : "Active",
              bg: user.plan === "free" ? "#fee2e2" : pb.bg,
              c: user.plan === "free" ? "#dc2626" : pb.c,
            },
  ...
  return (
    <div
      style={{ background: "#f0f4ff", minHeight: "100vh", paddingBottom: 120 }}
    >
      {/* Hero */}
      <div
        style={{
      ...
      >
        ...
        {/* Hostel row */}
        <div
          onClick={() => setShowHostelPicker(true)}
          style={{
            background: "rgba(255,255,255,.12)",
            borderRadius: 14,
            padding: "12px 16px",
        ...
        >
          <div>
            ...
            <div style={{ fontWeight: 700, color: "#fff" }}>
              {user.hostel || "Tap to set your hostel"}
            </div>
          </div>
          <span style={{ color: "rgba(255,255,255,.6)", fontSize: 20 }}>›</span>
        </div>
      ...
      <div style={{ padding: "0 16px 80px" }}>
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: "#78350f", marginTop: 1 }}>
                Required to create & join slashes · Tap to verify
              </div>
            </div>
        ...
        <Card style={{ padding: "12px 14px", marginTop: 14, marginBottom: 12 }}>
          ...
          >
            <div>
              <div style={{ fontSize: 11, color: "#64748b" }}>Hub</div>
              <div style={{ fontWeight: 700, color: "#1e293b", fontSize: 13 }}>
                {user.hubName || "—"}
              </div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 11, color: "#64748b" }}>Wallet</div>
              <div style={{ fontWeight: 700, color: "#1d4ed8", fontSize: 13 }}>
                {fmt(user.walletBalance)}
              </div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 11, color: "#64748b" }}>KYC</div>
              <div
                style={{
                  fontWeight: 700,
                  fontSize: 13,
      ...
      {showHubPicker && (
        <div
          ...
          onClick={(e) =>
            e.target === e.currentTarget && setShowHubPicker(false)
          }
        >
          <div
            style={{
          ...
          >
            ...
            <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 16 }}>
              Select your primary pickup location
            </div>
            {loadingProfileHubs ? (
              <div
                style={{
              ...
              allHubs.map((h) => (
                <div
                  ...
                  style={{
                    ...
                    background: user.hubId === h.id ? "#eff6ff" : "#fff",
                    marginBottom: 8,
                    cursor: "pointer",
                  }}
                >
                  <div
                    style={{
                      fontWeight: 700,
                      color: user.hubId === h.id ? "#1d4ed8" : "#1e293b",
                      fontSize: 14,
                    }}
                  >
                    {h.name}
          ...
          {hostelList.map((h) => (
            <button
              ...
              style={{
                ...
                alignItems: "center",
                gap: 10,
                background: user.hostel === h.name ? "#f0fdf4" : "#f8fafc",
                border: `1.5px solid ${user.hostel === h.name ? "#86efac" : "#e2e8f0"}`,
              }}
            >
