...
export function Subscription() {
  ...
  const upgrade = async (planId) => {
    ...
    try {
      const adminUsers = JSON.parse(Cookies.get("slashit_admin_users") || "[]");
      const synced = adminUsers.map((u) =>
        u.id === updated.id || u.email === updated.email
          ? { ...u, plan: planId }
          : u,
      );
      Cookies.set("slashit_admin_users", JSON.stringify(synced));
    } catch (e) {}
    toast.success(`Upgraded to ${plan.name}! 🎉`);
    setUpgrading(null);
  };
  const accents = {
    free: { border: "#e2e8f0", header: "#f8fafc", color: "#64748b" },
    student: { border: "#bfdbfe", header: "#eff6ff", color: "#1d4ed8" },
    premium: { border: "#fde68a", header: "#fef9c3", color: "#d97706" },
  };
  return (
    <div style={{ background: "#f0f4ff", minHeight: "100vh" }}>
      ...
      >
        {activePlans.map((plan) => {
          ...
          return (
            <Card
              key={plan.id}
              style={{
                border: `2px solid ${isCurrent ? acc.border : "#e2e8f0"}`,
                overflow: "hidden",
              }}
            >
              <div
                style={{
                  background: isCurrent ? acc.header : "#f8fafc",
                  ...
                  <div>
                    <div
                      style={{
                        fontWeight: 900,
                        fontSize: 16,
                        color: isCurrent ? acc.color : "#1e293b",
                      }}
                    >
                      {plan.name}
                    </div>
              ...
              <div style={{ padding: "12px 16px" }}>
                {plan.features.map((f) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: "5px 0",
                      fontSize: 12,
                      color: f.ok ? "#1e293b" : "#cbd5e1",
                    }}
                  >
                    <span
                      style={{
                        color: f.ok ? "#16a34a" : "#e2e8f0",
                        fontWeight: 700,
                      }}
                    >
                      {f.ok ? "✓" : "✗"}
                ...
                <div style={{ marginTop: 12 }}>
                  {isCurrent ? (
                    ...
                    >
                      Upgrade to {plan.name} — {fmt(plan.price)}/mo
                    </Btn>
                  ) : (
                    <Btn
                      full
        ...
        >
          Charged from wallet monthly. Cancel anytime.
        </div>
      </div>
      <LegalFooter />
    </div>
