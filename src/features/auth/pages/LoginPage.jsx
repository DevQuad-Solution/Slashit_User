...
export function LoginPage() {
  ...
  const prefillEmail = Cookies.get("slashit_prefill_email") || "";
  ...
  useEffect(() => {
    if (prefillEmail) Cookies.remove("slashit_prefill_email");
  }, []);
  ...
  const login = async () => {
    ...
    try {
      ...
      const user =
        res.data?.user || (typeof res.data === "object" ? res.data : {});
      if (!token)
      ...
      try {
        const pending = JSON.parse(
          Cookies.get("slashit_pending_join") || "null",
        );
        if (pending?.slashId) {
      ...
      if (
        ...
        toast.error(
          'Wrong email or password. If you just registered, complete email verification first — tap "Create account" and use the same email to finish setup.',
        );
      } else {
        toast.error(
    ...
    >
      <div
        style={{
          ...
          background: "#fff",
          borderRadius: 24,
          boxShadow: "0 8px 40px rgba(37,99,235,.13)",
      ...
      >
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 14, color: "#64748b" }}>
            Sign in to your SlashIt account
          </div>
        </div>
