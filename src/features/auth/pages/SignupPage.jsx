...
export function Signup() {
  ...
  const validate = () => {
    ...
    if (!form.name.trim()) e.name = "Full name is required";
      ...
      e.name = "Full name must be at least 6 characters";
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      toast.success(
        "Code sent! Check " +
          form.email +
          " — look in SPAM or JUNK if not in inbox.",
      );
    } catch (err) {
      ...
      if (
        ...
        toast.error(
          "This email already has an account. Tap Sign In to log in.",
        );
      } else if (
  ...
  const resendOtp = async () => {
    ...
    try {
      ...
      setResendCooldown(60);
      toast.success(
        "New code sent to " + form.email + ". Check spam/junk too.",
      );
    } catch (err) {
  ...
  const verifyEmail = async () => {
    ...
    try {
      ...
      const verifyRes = await api.auth.verifyCode(
        form.email,
        verifyCode.trim(),
        "signup",
      );
      ...
      const verifyToken =
        typeof verifyRes?.data === "string" && verifyRes.data.length > 10
          ? verifyRes.data
      ...
      Cookies.set("slashit_signup_email", form.email);
      Cookies.set("slashit_signup_pw", form.password);

      if (verifyToken) {
        // We have a real JWT — store and proceed to onboarding
        Cookies.set("slashit_verify_token", verifyToken);
        ...
        Cookies.remove("slashit_verify_token");
      ...
      if (raw.includes("Failed to fetch") || raw === "Failed to fetch") {
        ...
        toast.error(
          "Code is wrong or expired. Use the Resend button to get a new one.",
        );
        ...
        try {
          ...
          toast.error(
            "Your account was created but the session expired. Please sign in with your email and password.",
          );
          nav("/login");
        }
      } else if (
    ...
    return (
      ...
      >
        <div style={{ padding: "52px 24px 20px", textAlign: "center" }}>
          ...
          >
            We sent a{" "}
            <strong style={{ color: "#fff" }}>6-digit verification code</strong>{" "}
            to
            <br />
          ...
          >
            ⚠️ <strong>Check your SPAM / JUNK folder</strong> — most providers
            filter this email
            <br />
            <span style={{ fontWeight: 400, fontSize: 11, opacity: 0.9 }}>
              Subject line: look for "Verify", "OTP" or "SlashIt"
              <br />
        ...
        >
          <div style={{ marginBottom: 20 }}>
            ...
            <input
              value={verifyCode}
              onChange={(e) =>
                setVerifyCode(e.target.value.trim().replace(/\D/g, ""))
              }
              placeholder="000000"
              maxLength={6}
              inputMode="numeric"
    ...
    >
      ...
      >
        ...
        >
          ...
          {[
            ...
            },
            {
              key: "data",
              label:
                "I consent to SlashIt processing my personal data for account management and group buying coordination",
            },
            {
              key: "age",
              label:
                "I confirm I am 18 years or older and eligible to use this service",
            },
          ].map((item) => (
            <div
              key={item.key}
              onClick={() =>
                setForm((f) => ({
                  ...f,
                  ["consent_" + item.key]: !f["consent_" + item.key],
                }))
              }
              style={{
                display: "flex",
            ...
            >
              <input
                ...
                onChange={(e) => {
                  ...
                  setForm((f) => ({
                    ...f,
                    ["consent_" + item.key]: e.target.checked,
                  }));
                }}
        ...
        <Btn
          ...
          loading={loading}
          disabled={
            !form.consent_terms || !form.consent_data || !form.consent_age
          }
          onClick={submitForm}
        >
