...
export function SupportChat({ onClose }) {
  ...
  const getAttendantInfo = () => {
    try {
      ...
      if (hub?.attendantName)
        return { name: hub.attendantName, hubName: hub.name };
    } catch (e) {}
    ...
    if (hubId) {
      try {
        ...
        const marked = all.map((m) =>
          m.userId === userId && m.from === "attendant"
            ? { ...m, isRead: true }
            : m,
        );
  ...
  const send = async () => {
    ...
    try {
      const log = JSON.parse(Cookies.get("slashit_admin_chat_log") || "[]");
      log.unshift({
        ...msg,
        hubName: attInfo.hubName,
        attendantName: attInfo.name,
      });
      Cookies.set("slashit_admin_chat_log", JSON.stringify(log));
    } catch (e) {}
    try {
      const hubs = JSON.parse(Cookies.get("slashit_admin_hubs") || "[]");
      const hub = hubs.find((h) => h.id === hubId);
      ...
      if (attId) {
        ...
        const existing = JSON.parse(Cookies.get(key) || "[]");
        existing.unshift({
          id: "attnotif-" + Date.now(),
          type: "user_message",
        ...
        pushBrowserNotif(
          "💬 Message from " + (userName.split(" ")[0] || "Member"),
          text.slice(0, 80),
        );
      }
    } catch (e) {}
      ...
      >
        ...
        >
          {noHub && (
            ...
            >
              ...
              <div style={{ fontSize: 12, marginTop: 4 }}>
                Select a hub in your profile to chat with your hub attendant.
              </div>
            </div>
          )}
          {!noHub && messages.length === 0 && (
            ...
            >
              <div style={{ fontSize: 32, marginBottom: 8 }}>💬</div>
              <div style={{ fontWeight: 700, color: "#475569", fontSize: 13 }}>
                Chat with {attInfo.name}
              </div>
              <div style={{ fontSize: 12, marginTop: 4 }}>
                Ask about your order, report an issue, or get help with anything
                at {attInfo.hubName}.
              </div>
            </div>
          )}
        ...
        >
          <input
            ...
            placeholder={
              noHub ? "Select a hub first…" : `Message ${attInfo.name}…`
            }
            disabled={noHub}
          ...
          <button
            ...
            style={{
              ...
              height: 42,
              borderRadius: "50%",
              background: noHub || !input.trim() ? "#e2e8f0" : "#2563eb",
              color: noHub || !input.trim() ? "#94a3b8" : "#fff",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              fontSize: 18,
    ...
    const tick = () => {
      ...
      if (hubId) {
        const chat = chatStorage
          .load(hubId)
          .filter(
            (m) => m.userId === userId && m.from === "attendant" && !m.isRead,
          );
