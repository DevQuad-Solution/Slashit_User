...
function loadAdmin(key, fallback) {
  try {
    const raw = Cookies.get(ADMIN_NS + key);
    return raw !== null ? JSON.parse(raw) : fallback;
  } catch (e) {
    return fallback;
  }
...
export function getFlags() {
  ...
  try {
    const raw = Cookies.get(ADMIN_NS + "settings_flags");
...
export function getPlatformConfig() {
  try {
    const raw = Cookies.get(ADMIN_NS + "platform_config");
...
export const chatStorage = {
  ...
  load: (hubId) => {
    try {
      return JSON.parse(Cookies.get(`slashit_hub_chat_${hubId}`) || "[]");
    } catch (e) {
      return [];
    }
  },
...
export const msgStorage = {
  load: (userId) => {
    try {
      return JSON.parse(Cookies.get(`slashit_user_messages_${userId}`) || "[]");
    } catch (e) {
      return [];
    }
  },
  save: (userId, msgs) => {
    try {
      Cookies.set(`slashit_user_messages_${userId}`, JSON.stringify(msgs));
    } catch (e) {}
  },
  push: (userId, msg) => {
    try {
      msgStorage.save(userId, [msg, ...msgStorage.load(userId)]);
    } catch (e) {}
  },
  markRead: (userId, msgId) => {
    msgStorage.save(
      ...
      msgStorage
        .load(userId)
        .map((m) => (m.id === msgId ? { ...m, isRead: true } : m)),
    );
  },
  markAllRead: (userId) => {
    msgStorage.save(
      userId,
      msgStorage.load(userId).map((m) => ({ ...m, isRead: true })),
    );
  },
};
...
export const pollStorage = {
  load: () => {
    try {
      return JSON.parse(Cookies.get(SHARED_POLLS_KEY) || "[]");
    } catch (e) {
      return [];
    }
  },
  save: (polls) => {
    try {
      Cookies.set(SHARED_POLLS_KEY, JSON.stringify(polls));
    } catch (e) {}
  },
  add: (poll) => {
    pollStorage.save([poll, ...pollStorage.load()]);
  },
  update: (pollId, changes) => {
    pollStorage.save(
      pollStorage
        .load()
        .map((p) => (p.id === pollId ? { ...p, ...changes } : p)),
    );
  },
};
